(function () {
  ...
  // Replace the API key or the complete URL/attribution for your licensed commercial tile provider.
  var TILE_PROVIDER_CONFIG = {
    url: "https://api.maptiler.com/maps/streets-v4/{z}/{x}/{y}.png?key=YOUR_MAPTILER_API_KEY",
  ...
  function renderCreatedPolicies() {
    ...
    body.innerHTML = managed.map(function (rule) {
      ...
      var groupNames = (rule.groups || []).map(function (reference) {
        ...
        return group ? group.name : reference.id;
      });
  ...
  function initializeMap() {
    ...
    window.L.tileLayer(TILE_PROVIDER_CONFIG.url, {
      ...
      text("map-status", "Basemap tiles could not load. Confirm the configured commercial tile URL and credentials, and check whether MyGeotab is blocking tile requests." + mapTimeZoneWarning());
    }).addTo(fleetMap);
  ...
  function renderMap() {
    initializeMap();
    if (!fleetMap) {
      text("map-status", "The bundled leaflet.js map library did not load, or MyGeotab blocked the configured basemap tile requests." + mapTimeZoneWarning());
      return;
    }
    ...
    text("map-status", zone
      ? (zone.name || "Selected zone") + ": " + latestRows.filter(function (row) { return row.hasPosition; }).length +
        " positioned vehicle(s), " + triggeredCount + " active trigger(s)." + mapTimeZoneWarning()
      : "Select a zone to display its boundary and monitored vehicles." + mapTimeZoneWarning());
  ...
  function loadReferenceData(force) {
    ...
    referenceDataPromise = (async function () {
      ...
      try {
        ...
        populateSelect(byId("group-select"), groups, "All accessible vehicles — monitoring only", function (group) {
          return group.name;
        });
  ...
  function updatePolicyFormState() {
    ...
    var ready = Boolean(zone.value && group.value && name.value.trim() && trigger.value &&
    ...
    create.title = ready ? "Create this policy in MyGeotab"
      ...
      : !group.value ? "Select a specific vehicle group"

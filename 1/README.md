...
The interactive Leaflet map uses OpenStreetMap tiles. Its zone polygon and
vehicle markers are generated only from live `Zone` and `DeviceStatusInfo`
records. If the map library or tiles are blocked, the results table retains
direct Google Maps links for each live position.

(function () {
  ...
  var zones = [];
  var rules = [];
  var recentlyCreatedRules = [];
  var workTimes = [];
  var distributionLists = [];
  var monitoredPolicyIds = null;
  var latestRows = [];
  var fleetMap = null;
  var zoneLayer = null;
  ...
  async function registerPolicyDelivery(payload, webhookUrl) {
    ...
    try {
      ...
      throw new Error("The notification service URL is not valid.");
    }
    if (endpoint.protocol !== "https:" || endpoint.pathname.replace(/\/+$/, "") !== "/api/policies/register") {
      throw new Error("Use the deployed HTTPS notification endpoint ending in /api/policies/register.");
    }
    var session = await getApiSession();
    if (!session || !session.database || !session.userName || !session.sessionId) {
      throw new Error("The current MyGeotab session could not be included with the notification registration.");
    }
    payload.registrationSession = {
    ...
    if (!response.ok) {
      ...
      throw new Error(responseBody.error || "Notification service returned HTTP " + response.status);
    }
  ...
  function monitoredPolicies() {
    var managed = managedPolicies();
    if (monitoredPolicyIds === null && managed.length) {
      monitoredPolicyIds = managed.map(function (rule) { return rule.id; });
    }
    if (monitoredPolicyIds === null) return [];
    return managed.filter(function (rule) { return monitoredPolicyIds.indexOf(rule.id) !== -1; });
  }
  ...
  function triggeringCreatedPolicies(row, device) {
    ...
    return monitoredPolicies().filter(function (rule) {
  ...
  function renderCreatedPolicies() {
    ...
    if (!managed.length) {
      body.innerHTML = '<tr><td colspan="9" class="empty">No policies created by this add-in were found.</td></tr>';
      text("created-policies-status", "0 created policies");
      byId("select-all-policies").checked = false;
      return;
    }
    if (monitoredPolicyIds === null) monitoredPolicyIds = managed.map(function (rule) { return rule.id; });
    monitoredPolicyIds = monitoredPolicyIds.filter(function (id) {
      return managed.some(function (rule) { return rule.id === id; });
    });
    body.innerHTML = managed.map(function (rule) {
      ...
      return "<tr>" +
        '<td><input class="policy-monitor-check" type="checkbox" data-policy-id="' + escapeHtml(rule.id) +
        '" aria-label="Monitor ' + escapeHtml(rule.name || "policy") + '"' +
        (monitoredPolicyIds.indexOf(rule.id) !== -1 ? " checked" : "") + "></td>" +
        ...
        '<td><button class="policy-delete" type="button" data-policy-id="' + escapeHtml(rule.id) +
        '">Delete</button></td>' +
        "</tr>";
    }).join("");
    byId("select-all-policies").checked = monitoredPolicyIds.length === managed.length;
    text("created-policies-status", monitoredPolicyIds.length + " of " + managed.length +
      " created polic" + (managed.length === 1 ? "y" : "ies") + " selected for monitoring");
  }
  async function loadPolicyData() {
    ...
    try {
      var loadedRules = await apiCall("Get", {
        typeName: "Rule",
        search: { baseType: "Custom" }
      }) || [];
      rules = loadedRules.concat(recentlyCreatedRules.filter(function (recentRule) {
        return !loadedRules.some(function (loadedRule) { return loadedRule.id === recentRule.id; });
      }));
      var policyZoneIds = [];
  ...
  function renderMap() {
    ...
    var policyZones = monitoredPolicies().map(policyZone).filter(Boolean);
    ...
    text("map-status", monitoredPolicies().length
      ? monitoredPolicies().length + " selected polic" + (monitoredPolicies().length === 1 ? "y" : "ies") +
  ...
  async function refresh() {
    ...
    if (!zoneId && !monitoredPolicies().length) {
      showMessage("Select at least one created policy to monitor, or select a draft Geotab zone.", false);
      latestRows = [];
      renderRows();
      renderMap();
      return;
    ...
    try {
      ...
      showMessage("Showing " + selectedDevices.length + " accessible vehicle" +
        (selectedDevices.length === 1 ? "" : "s") + " against " + monitoredPolicies().length +
        " selected polic" + (monitoredPolicies().length === 1 ? "y" : "ies") + "." +
  ...
  async function createPolicy() {
    ...
    var confirmed = window.confirm(
      ...
      (notifications.backOfficeEmails.length && !needsExternalDelivery
        ? "\n\nBack-office email alerts will be created as a native MyGeotab distribution list."
        : "") +
      (needsExternalDelivery
        ? "\nSelected email delivery and map/location content will be registered with the Node notification service."
        : "")
    );
    if (!confirmed) return;
    ...
    try {
      ...
      entity.id = ruleId;
      rules.push(entity);
      recentlyCreatedRules.push(entity);
      if (monitoredPolicyIds === null) monitoredPolicyIds = [];
      monitoredPolicyIds.push(ruleId);
      renderCreatedPolicies();
      var distributionListId = needsExternalDelivery ? null : await createNativeEmailDistribution(
        name,
        ruleId,
      ...
      byId("created-policies-heading").scrollIntoView({ behavior: "smooth", block: "start" });
    } catch (error) {
  ...
  async function deletePolicy(ruleId, button) {
    var rule = managedPolicies().find(function (item) { return item.id === ruleId; });
    if (!rule) {
      showPolicyResult("The selected policy is no longer available. Refresh the policy register.", "error");
      return;
    }
    if (!window.confirm(
      "Delete the MyGeotab policy “" + (rule.name || rule.id) + "”?\n\n" +
      "This removes the exception rule and its notification distribution. This cannot be undone."
    )) return;
    button.disabled = true;
    showPolicyResult("Deleting policy “" + (rule.name || rule.id) + "”…", "");
    var workCondition = findCondition(rule.condition, "RuleWorkHours") ||
      findCondition(rule.condition, "AfterRuleWorkHours");
    var workTimeId = workCondition && workCondition.workTime ? workCondition.workTime.id : null;
    var linkedLists = distributionLists.filter(function (list) {
      return (list.rules || []).some(function (reference) { return reference.id === ruleId; });
    });
    try {
      await apiCall("Remove", { typeName: "Rule", entity: { id: ruleId } });
      for (var i = 0; i < linkedLists.length; i += 1) {
        await apiCall("Remove", { typeName: "DistributionList", entity: { id: linkedLists[i].id } });
      }
      var workTimeUsedElsewhere = workTimeId && managedPolicies().some(function (item) {
        if (item.id === ruleId) return false;
        var condition = findCondition(item.condition, "RuleWorkHours") ||
          findCondition(item.condition, "AfterRuleWorkHours");
        return condition && condition.workTime && condition.workTime.id === workTimeId;
      });
      if (workTimeId && !workTimeUsedElsewhere) {
        await apiCall("Remove", { typeName: "WorkTime", entity: { id: workTimeId } });
      }
      monitoredPolicyIds = (monitoredPolicyIds || []).filter(function (id) { return id !== ruleId; });
      recentlyCreatedRules = recentlyCreatedRules.filter(function (item) { return item.id !== ruleId; });
      showPolicyResult("Policy “" + (rule.name || rule.id) + "” was deleted from MyGeotab.", "success");
      await loadPolicyData();
      await refresh();
    } catch (error) {
      showPolicyResult("Policy deletion failed: " + errorText(error) +
        ". Refresh the register to confirm which records remain.", "error");
      await loadPolicyData();
    } finally {
      button.disabled = false;
    }
  ...
  function validNotificationServiceUrl(value) {
    try {
      var endpoint = new URL(value);
      return endpoint.protocol === "https:" &&
        endpoint.pathname.replace(/\/+$/, "") === "/api/policies/register";
    } catch (error) {
      return false;
    }
  }
  ...
  function updatePolicyFormState() {
    ...
    var needsExternalDelivery = notifications.enabled &&
      (notifications.notifyAssignedDriver || notifications.includeLocationLink || notifications.attachMapImage);
    var deliveryValid = !needsExternalDelivery || validNotificationServiceUrl(notificationWebhookUrl.value.trim());
    var recipientParts = [];
    ...
    create.title = ready ? "Create this policy in MyGeotab"
      ...
      : !deliveryValid ? "Enter the HTTPS Node service URL ending in /api/policies/register"
  ...
  function bindEvents() {
    ...
    byId("select-all-policies").addEventListener("change", function () {
      monitoredPolicyIds = byId("select-all-policies").checked
        ? managedPolicies().map(function (rule) { return rule.id; }) : [];
      renderCreatedPolicies();
      refresh();
    });
    byId("created-policies-body").addEventListener("change", function (event) {
      var target = event.target;
      if (!target || String(target.className).indexOf("policy-monitor-check") === -1) return;
      var ruleId = target.getAttribute("data-policy-id");
      monitoredPolicyIds = monitoredPolicyIds || [];
      if (target.checked && monitoredPolicyIds.indexOf(ruleId) === -1) monitoredPolicyIds.push(ruleId);
      if (!target.checked) {
        monitoredPolicyIds = monitoredPolicyIds.filter(function (id) { return id !== ruleId; });
      }
      renderCreatedPolicies();
    ...
    byId("created-policies-body").addEventListener("click", function (event) {
      var target = event.target;
      if (!target || String(target.className).indexOf("policy-delete") === -1) return;
      deletePolicy(target.getAttribute("data-policy-id"), target);
    });
